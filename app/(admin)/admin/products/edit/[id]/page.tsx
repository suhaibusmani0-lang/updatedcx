...
import { Loader2, AlertCircle, CheckCircle, X, Plus, Trash2, Upload } from "lucide-react";
...
export default function EditProductPage() {
  ...
  const [existingVideo, setExistingVideo] = useState<{ url: string; public_id: string; duration?: number; format?: string } | null>(null);
  ...
  const [variants, setVariants] = useState<Array<{ size: string; color: string; stock: string; sku: string; price: string; salePrice: string; image: string }>>([
    { size: "", color: "", stock: "0", sku: "", price: "", salePrice: "", image: "" },
  ]);
        ...
        if (data.ok && data.data) {
          ...
          setVariants(
            Array.isArray(product.variants) && product.variants.length > 0
              ...
              : [{ size: "", color: "", stock: "0", sku: "", price: "" }]
          );
          setExistingImages(product.images || []);
          setExistingVideo(product.video?.url ? product.video : null);
        } else {
  ...
  const handleProductVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) { setSubmitError("Product video must be MP4, WebM, or MOV."); e.target.value = ""; return; }
    if (file.size > 50 * 1024 * 1024) { setSubmitError("Product video must be 50MB or smaller."); e.target.value = ""; return; }
    ...
    probe.onloadedmetadata = () => {
      ...
      if (!Number.isFinite(duration) || duration > 30.05) {
        URL.revokeObjectURL(preview); setSubmitError("Product video must be 30 seconds or shorter."); e.target.value = ""; return;
      }
      if (newVideoPreview) URL.revokeObjectURL(newVideoPreview);
      setNewVideo(file); setNewVideoPreview(preview); setRemoveExistingVideo(false); setIsDirty(true); setSubmitError(null);
    };
    probe.onerror = () => { URL.revokeObjectURL(probe.src); URL.revokeObjectURL(preview); setSubmitError("Could not read the selected video."); e.target.value = ""; };
    probe.src = preview;
  };

  const removeNewProductVideo = () => {
    if (newVideoPreview) URL.revokeObjectURL(newVideoPreview);
    setNewVideo(null); setNewVideoPreview(""); setIsDirty(true);
  };
  ...
  const updateVariant = (index: number, field: keyof (typeof variants)[number], value: string) => {
  ...
  const submitHandler = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const [uploadedImages, uploadedVideo] = await Promise.all([
        Promise.all(newImages.map((file) => uploadToCloudinary(file, "products", "image", signal))),
        newVideo ? uploadToCloudinary(newVideo, "products/videos", "video", signal) : Promise.resolve(null),
      ]);
      ...
      const filteredVariants = variants
        .filter((variant) => variant.size || variant.color || variant.sku || variant.stock || variant.price)
        .map((variant) => ({
      ...
      const payload = {
        ...
        newImages: uploadedImages.map((image) => ({ url: image.url, public_id: image.public_id })),
      ...
      if (!res.ok || !data.ok) throw new Error(data.message || "Failed to update product");
      ...
      <div className="p-6">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <Loader2 size={40} className="animate-spin mx-auto text-[#AEAA9B] mb-4" />
        ...
        <form onSubmit={submitHandler} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#AEAA9B] transition ${
          ...
          <div className="rounded-lg border border-gray-200 p-4 bg-gray-50">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500">Manage SKU, stock, and price overrides for each combination.</p>
              </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-5">
            <div className="mb-3">
              ...
              <p className="mt-1 text-xs text-gray-500">Optional · replace or remove · maximum 30 seconds · max 50MB.</p>
            </div>
            {existingVideo && !removeExistingVideo && !newVideoPreview && (
              <div className="mb-4 max-w-xl">
                <div className="aspect-video overflow-hidden rounded-lg border bg-black"><video src={existingVideo.url} controls className="h-full w-full object-contain" /></div>
                <div className="mt-2 flex items-center gap-3">
                  <button type="button" onClick={() => { setRemoveExistingVideo(true); setIsDirty(true); }} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50">Remove current video</button>
                  <span className="text-xs text-gray-500">{existingVideo.duration ? `${existingVideo.duration.toFixed(1)}s` : "Product video"}</span>
                </div>
              </div>
            )}
            {removeExistingVideo && !newVideoPreview && <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">Current video will be removed when you save.</div>}
            {newVideoPreview ? (
              <div className="relative aspect-video max-w-xl overflow-hidden rounded-lg border bg-black"><video src={newVideoPreview} controls muted className="h-full w-full object-contain" /><button type="button" onClick={removeNewProductVideo} aria-label="Remove selected product video" className="absolute right-2 top-2 rounded-full bg-red-500 p-2 text-white hover:bg-red-600"><X size={16} /></button></div>
            ) : (
              <label className="flex max-w-xl cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-white px-6 py-8 text-center transition-colors hover:border-gray-500">
                <div><Upload className="mx-auto mb-2 text-gray-500" size={24} /><span className="block text-sm font-medium">Choose replacement video</span><span className="mt-1 block text-xs text-gray-400">MP4, WebM or MOV · maximum 30 seconds</span></div>
