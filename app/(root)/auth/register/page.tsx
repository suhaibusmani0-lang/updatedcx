...
const formSchema = z
  .object({
    ...
    phone: z.string().min(10, "Phone number must be at least 10 digits").max(16, "Phone number is too long"),
...
export default function RegisterPage() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      // The firebase helper intentionally exposes a loosely typed fallback when
      // Firebase is not configured. After the runtime guard above, it is safe to
      // pass the configured instance to Firebase Auth APIs.
  ...
  return (
    <div className="flex justify-center items-center min-h-screen px-4 sm:px-6">
      <div className="bg-white shadow-lg rounded-xl p-4 sm:p-6 w-full max-w-md boxshadow-lg">
        ...
        <form onSubmit={form.handleSubmit(handleSubmitRegister)} className="space-y-3 sm:space-y-4">
          <div>
            <label className="text-sm sm:text-base">Full Name</label>
            <input {...form.register("name")} placeholder="Enter your name" className="w-full border rounded px-3 py-2 mt-1 text-sm sm:text-base" />
            {form.formState.errors.name && <p className="text-red-500 text-xs mt-1">{form.formState.errors.name.message}</p>}
          </div>

          <div>
            <label className="text-sm sm:text-base">Mobile Number</label>
            <input type="tel" {...form.register("phone")} placeholder="+91 98765 43210" className="w-full border rounded px-3 py-2 mt-1 text-sm sm:text-base" />
            {form.formState.errors.phone && <p className="text-red-500 text-xs mt-1">{form.formState.errors.phone.message}</p>}
          </div>

          <div>
            <label className="text-sm sm:text-base">Email</label>
            <input type="email" {...form.register("email")} placeholder="example@gmail.com" className="w-full border rounded px-3 py-2 mt-1 text-sm sm:text-base" />
            {form.formState.errors.email && <p className="text-red-500 text-xs mt-1">{form.formState.errors.email.message}</p>}
          </div>

          <div>
            <label className="text-sm sm:text-base">Password</label>
            <input type="password" {...form.register("password")} placeholder="******" className="w-full border rounded px-3 py-2 mt-1 text-sm sm:text-base" />
          ...
          <div>
            ...
            <input type="password" {...form.register("confirmPassword")} placeholder="******" className="w-full border rounded px-3 py-2 mt-1 text-sm sm:text-base" />
          ...
          <ButtonLoading type="submit" loading={loading} text="Create Account" className="cursor-pointer w-full text-sm sm:text-base py-2 sm:py-3" />

          <button type="button" onClick={handleGoogleRegister} disabled={googleLoading || loading} className="flex w-full items-center justify-center gap-2 rounded-none border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:opacity-60">
