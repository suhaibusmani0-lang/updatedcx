...
// 🚀 SPEED OPTIMIZATION: Next.js Cache Import Kiya Hai
...
async function getCategoryData(slug: string, searchParams: SearchParams) {
  // 🚀 SPEED OPTIMIZATION: Database Query ko cache mein daal diya
  const getCachedData = unstable_cache(
    async () => {
      try {
        await connectDB();

        // FIX: Slug ko hamesha lowercase karke search karenge taaki case issue na aaye
        ...
        // 🚀 NAYA: Fetch sub-categories agar yeh root category hai
        const subCategories = await CategoryModel.find({ 
          parent: (category as any)._id, 
          isActive: true 
        }).select('_id').lean();
        
        // Root aur uski saari sub-categories ki IDs ka array banalo
        const categoryIds = [(category as any)._id, ...subCategories.map(c => c._id)];

        const baseQuery: Record<string, unknown> = {
          category: { $in: categoryIds }, // Sirf ek nahi, balki sub-categories bhi check hongi
          isActive: true,
        };
        ...
        // FIX: Error ko throw kar rahe hain taaki failure cache na ho jaye
        throw new Error("Failed to fetch category data");
      }
    },
    // FIX: Cache key ko bhi lowercase mein set kar diya
...
export default async function CategoryPage({
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] pb-20 lg:pb-0">
      ...
      {/* 🚀 CATEGORY HEADER SECTION (Updated Sizes & Full Width) */}
