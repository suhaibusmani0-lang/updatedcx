...
export default function CheckoutPage() {
  ...
  // Naye Loading States Auto-fill ke liye
  ...
  // COD charge integration (Changed from 69 to 59)
  const codCharge = paymentMethod === "COD" ? 59 : 0;
  
  const giftWrapCharge = isGiftWrap ? 99 : 0;
  const total = subtotal + shipping + codCharge - discount - paymentDiscount + giftWrapCharge;
      ...
      const fetchLocation = async () => {
        ...
        try {
          ...
          if (data && data[0].Status === "Success") {
            ...
            form.setValue("city", postOffice.District || postOffice.Block, { shouldValidate: true });
  ...
  useEffect(() => {
    if (billingPincode?.length === 6 && !useSameBillingAddress) {
      const fetchBillingLocation = async () => {
        setIsFetchingBillingPincode(true);
        try {
          const res = await fetch(`https://api.postalpincode.in/pincode/${billingPincode}`);
          const data = await res.json();
          if (data && data[0].Status === "Success") {
            const postOffice = data[0].PostOffice[0];
            form.setValue("billingCity", postOffice.District || postOffice.Block, { shouldValidate: true });
            form.setValue("billingState", postOffice.State, { shouldValidate: true });
          }
        } catch (error) {
          console.error("Error fetching billing pincode details", error);
        } finally {
          setIsFetchingBillingPincode(false);
        }
      };
      fetchBillingLocation();
    }
  }, [billingPincode, useSameBillingAddress, form]);
  // ---------------------------------------------
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const data = await res.json();
        // Bhale hi backend koi bhi charge bheje, hum client pe 0 (Free) hi dikhayenge
    ...
    try {
      const payload = {
        ...
        items: items.map((item) => ({
          ...
          sku: item.variant?.sku || String(item.id),
          units: item.qty,
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (formData.paymentMethod === "COD") {
        dispatch(clearCart());
        await fetch("/api/cart?clear=true", { method: "DELETE" }).catch(() => null);
        ...
        modal: {
          ondismiss: async () => {
            ...
            try {
              ...
              console.error("Failed to cancel order status on backend", err);
            }
          },
        },
      };
  ...
  if (!items.length) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 px-4">
        ...
        <Link href="/" className="text-xs tracking-widest uppercase text-[#1A1A1A] border-b border-[#1A1A1A] pb-0.5 hover:text-[#AEAA9B] hover:border-[#AEAA9B] transition-colors">
          Continue Shopping
        </Link>
      </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Pincode" error={form.formState.errors.pincode?.message}>
                  <div className="relative">
                    ...
                    {isFetchingPincode && (
                      <span className="absolute right-3 top-[13px] text-[10px] uppercase font-bold text-[#AEAA9B]">
                        Fetching...
                      </span>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-[#E3D9C9] p-5 sm:p-6 mt-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <label className="flex items-center gap-2 text-sm text-[#8B6F52]">
                  <input type="checkbox" className="accent-[#AEAA9B]" {...form.register("useSameBillingAddress")} />
              ...
              {!useSameBillingAddress && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {/* Billing Pincode with Auto-fill Indicator */}
                  <Field label="Pincode">
                    <div className="relative">
            ...
            <div className="bg-white rounded-2xl border border-[#E3D9C9] p-5 sm:p-6">
              ...
              <div className="space-y-3">
                <label className="flex items-start gap-3 rounded-xl border border-[#E3D9C9] bg-[#F8F3EA] p-4 cursor-pointer">
                  <input type="radio" value="COD" className="accent-[#AEAA9B] mt-1" {...form.register("paymentMethod")} />
                  <div>
                ...
                <label className="flex items-start gap-3 rounded-xl border border-[#E3D9C9] bg-[#F8F3EA] p-4 cursor-pointer">
                  <input type="radio" value="Razorpay" className="accent-[#AEAA9B] mt-1" {...form.register("paymentMethod")} />
                  <div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#1A1A1A] text-white text-xs tracking-widest uppercase py-4 rounded-xl hover:bg-[#AEAA9B] transition-colors disabled:opacity-60 font-semibold"
            >
            ...
            {!auth && (
              <p className="text-center text-xs text-[#8B6F52]">
                You need to{" "}
                <Link href="/auth/login?callback=/checkout" className="text-[#AEAA9B] underline">login</Link>
                {" "}to place an order.
              </p>
            )}
          </form>
            ...
            <div className="bg-white rounded-2xl border border-[#E3D9C9] p-5">
              ...
              <div className="space-y-3 mb-4 rounded-xl border border-[#E3D9C9] bg-[#F8F3EA] p-3 text-sm text-[#8B6F52]">
                {paymentMethod === "Razorpay" ? (
                  ...
                  <p>Cash on delivery includes an additional ₹59 handling fee.</p>
                )}
              </div>

              
              <div className="flex items-center space-x-3 p-3 border border-[#E3D9C9] rounded-xl mb-4 bg-white">
                <input
                  type="checkbox"
                  id="giftWrap"
                  checked={isGiftWrap}
                ...
                />
                <label htmlFor="giftWrap" className="flex flex-col cursor-pointer select-none">
                  <span className="font-semibold text-sm text-[#1A1A1A]">Add Gift Wrap (+₹99)</span>
                  <span className="text-xs text-[#8B6F52]">Make it special with our premium packaging.</span>
                </label>
              </div>
              ...
              <div className="border-t border-[#E3D9C9] mt-4 pt-4 space-y-2">
                ...
                {/* Shipping Display Logic with Strikethrough & FREE */}
                <div className="flex justify-between text-sm text-[#8B6F52]">
                  <span>Shipping</span>
            ...
            <div className="bg-white rounded-2xl border border-[#E3D9C9] p-5">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 border border-[#E3D9C9] rounded-lg px-3 py-2 text-sm text-[#1A1A1A] placeholder:text-[#8B6F52]/50 outline-none focus:border-[#AEAA9B] tracking-widest uppercase"
                />
                <button
                  type="button"
                  onClick={applyCoupon}
                  className="bg-[#1A1A1A] text-white px-4 py-2 rounded-lg text-xs tracking-widest uppercase hover:bg-[#AEAA9B] transition-colors"
                >
                  Apply
                </button>
              </div>
            ...
            <div className="bg-[#F1EBE1] rounded-2xl border border-[#E3D9C9] p-4 space-y-3">
              {[
                ...
                <div key={text} className="flex items-center gap-2.5 text-xs text-[#8B6F52]">
                  <Icon size={14} className="text-[#AEAA9B] shrink-0" />
                  {text}
                </div>
              ))}
            </div>
...
function input() {
  return "w-full border border-[#E3D9C9] rounded-lg px-3 py-2.5 text-sm text-[#1A1A1A] outline-none focus:border-[#AEAA9B] transition-colors bg-white placeholder:text-[#8B6F52]/40";
}
