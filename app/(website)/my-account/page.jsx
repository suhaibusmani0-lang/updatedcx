...
import { login, logout, updateProfileData } from "@/store/reducer/authReducer";
...
export default function MyAccount() {
    ...
    const loadCurrentUser = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setIsEditingProfile(!currentUser.name || currentUser.name === "Customer");
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (tabFromUrl && ["profile", "orders", "addresses", "wishlist", "settings"].includes(tabFromUrl)) {
      setActiveTab(tabFromUrl);
    } else if (orderIdFromQuery) {
      setActiveTab("orders");
    }
  }, [searchParams, orderIdFromQuery]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [activeTab, auth, authLoading, orderIdFromQuery]);
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      if (res.ok) {
        ...
        dispatch(updateProfileData({ name: profileForm.name, phone: profileForm.phone }));
      } else {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      if (!auth?.hasPassword) {
        setPassMessage({
          type: "error",
          text: "Your account does not have a password. Please continue using your current sign-in method.",
        });
        return;
      }
  ...
  const handleTabChange = (tabId) => {
    ...
    if (tabId !== "orders") params.delete("orderId");
  ...
  if (authLoading || !auth) {
    return (
      <div className="min-h-screen bg-[#FAF7F2] flex items-center justify-center px-4">
        <div className="text-sm text-[#1A1A1A]/60">Loading your account…</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAF7F2] py-8 px-4 sm:px-6 md:px-10">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-[#1A1A1A]/60 mt-2">Welcome back, {displayName}</p>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm p-4 sticky top-24">
              ...
              <div className="flex items-center gap-3 pb-4 border-b border-gray-100 mb-4">
                <div className="w-12 h-12 rounded-full bg-[#AEAA9B] flex items-center justify-center text-white font-bold text-lg">
                  {displayName !== "Complete your profile" ? displayName[0].toUpperCase() : "•"}
                </div>
                <div>
                  <p className="font-semibold text-[#1A1A1A]">{displayName}</p>
                  <p className="text-xs text-[#1A1A1A]/60">{auth?.email || "Not provided"}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!isEditingProfile ? (
                    <button 
                      ...
                      className="flex items-center gap-2 text-[#AEAA9B] hover:text-[#A06245] transition-colors"
                    >
                      <Edit2 size={16} />
                    ...
                    <button 
                      onClick={() => {
                        setIsEditingProfile(false);
                        setProfileForm({ name: auth.name || "", phone: auth.phone || "" });
                      }}
                ...
                {isEditingProfile ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#AEAA9B] focus:ring-2 focus:ring-[#AEAA9B]/20 outline-none transition-all"
                      />
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-[#1A1A1A]/40 mt-1">
                        {auth?.authProvider === "mobile" && !auth?.email
                          ? "Add an email through your account support flow when needed."
                    ...
                    <div className="md:col-span-2 flex justify-end mt-2">
                      <button
                        ...
                        className="px-6 py-3 bg-[#AEAA9B] text-white rounded-xl font-medium hover:bg-[#9B9789] transition-colors disabled:opacity-70"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-[#1A1A1A]/60 mb-2">Full Name</label>
                      <p className="text-[#1A1A1A] font-medium">{auth?.name || "Not provided"}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#1A1A1A]/60 mb-2">Email Address</label>
                      <p className="text-[#1A1A1A] font-medium">{auth?.email || "Not provided"}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#1A1A1A]/60 mb-2">Phone Number</label>
                      <p className="text-[#1A1A1A] font-medium">{auth?.phone || "Not provided"}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-8 pt-6 border-t border-gray-100">
                  <h3 className="text-lg font-semibold text-[#1A1A1A] mb-2">Change Password</h3>
                  {!auth?.hasPassword ? (
                    <p className="text-sm text-[#1A1A1A]/60">
                      Password changes are unavailable because this account uses {auth?.authProvider === "google" ? "Google sign-in" : "mobile sign-in"}.
                    </p>
                  ) : (
                  <>
                  ...
                  <button
                    ...
                    className={`mt-4 px-6 py-3 rounded-xl font-medium transition-colors ${
                      ...
                      : "bg-[#AEAA9B] text-white hover:bg-[#9B9789]"
                    }`}
                  >
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8">
                ...
                {loading ? (
                  ...
                  <div className="space-y-4">
                    {orders.map((order) => (
                      <div key={order._id} className="border border-gray-100 rounded-xl p-4 hover:border-[#AEAA9B]/30 transition-colors">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                          <div className="flex items-start gap-4">
                            <div className="w-12 h-12 rounded-lg bg-gray-50 flex items-center justify-center">
                              <Package size={20} className="text-[#AEAA9B]" />
                            </div>
                            <div>
                        ...
                        <div className="flex flex-wrap justify-end gap-2 mt-3">
                          <a
                            ...
                            className="px-4 py-2 border border-[#AEAA9B] text-[#AEAA9B] rounded-xl text-sm font-medium hover:bg-[#AEAA9B] hover:text-white transition-colors inline-flex items-center gap-1.5"
                          ...
                          <a
                            href={`/api/orders/${order._id}/download`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-4 py-2 border border-[#1A1A1A] text-[#1A1A1A] rounded-xl text-sm font-medium hover:bg-[#1A1A1A] hover:text-white transition-colors inline-flex items-center gap-1.5"
                          ...
                          <button
                            ...
                            className="px-4 py-2 bg-[#AEAA9B] text-white rounded-xl text-sm font-medium hover:bg-[#9B9789] transition-colors"
                          >
                            Reorder
                          </button>
                        </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-[#AEAA9B] text-white rounded-xl font-medium hover:bg-[#9B9789] transition-colors"
                  >
                    <MapPin size={16} />
                  ...
                  <div className="bg-[#FAF7F2] rounded-2xl p-5">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                      <button
                        ...
                        className="px-5 py-3 bg-[#AEAA9B] text-white rounded-xl font-medium hover:bg-[#9B9789] transition-colors"
                      >
                ...
                {addresses.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {addresses.map((address, idx) => (
                      <div key={idx} className="border border-gray-100 rounded-xl p-4 hover:border-[#AEAA9B]/30 transition-colors">
                        <div className="flex items-start justify-between mb-3 gap-3">
                          <div>
                            <span className="px-2 py-1 bg-gray-100 rounded-none text-xs font-medium text-[#1A1A1A]">{address.type}</span>
                            {address.isDefault && (
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="rounded-xl border border-gray-200 px-3 py-2 text-sm text-[#1A1A1A] hover:border-[#AEAA9B] hover:text-[#AEAA9B] transition-colors"
                            >
                              Edit
                            </button>
                            <button
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8">
                ...
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {wishlist.map((item) => (
                      <div key={item._id} className="border border-gray-100 rounded-xl overflow-hidden hover:border-[#AEAA9B]/30 transition-colors group">
                        <div className="relative aspect-square bg-gray-50">
                          ...
                          <button
                            ...
                            className="absolute top-3 right-3 rounded-full bg-white p-2 shadow-sm text-[#1A1A1A] hover:bg-[#AEAA9B] hover:text-white transition-colors"
                          >
                            Remove
                          </button>
                        </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center">
                        <CreditCard size={20} className="text-[#AEAA9B]" />
                      </div>
                      <div>
                  ...
                  <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center">
                        <MapPin size={20} className="text-[#AEAA9B]" />
                      </div>
                      <div>
                  ...
                  <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center">
                        <Settings size={20} className="text-[#AEAA9B]" />
                      </div>
                      <div>
                  ...
                  <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center">
                        <User size={20} className="text-[#AEAA9B]" />
                      </div>
                      <div>
