...
interface Product {
  ...
  sizes: string[];
  colors?: string[];
  variants?: Array<{ size?: string; color?: string; stock?: number; sku?: string; price?: number | null }>;
  ...
  return (
    <div className="min-h-screen bg-white text-[#1A1A1A]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 text-sm text-gray-500">
        <Link href="/" className="hover:text-[#AEAA9B]">
          Home
        </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            <div className="inline-block bg-[#AEAA9B] text-white text-xs font-bold px-3 py-1 rounded-full">
              New Arrival
            </div>
            ...
            <div className="space-y-1">
              <p className="text-2xl font-semibold text-[#AEAA9B]">
                MRP: ₹{" "}
                {(product.salePrice != null && product.salePrice < product.price ? product.salePrice : product.price).toLocaleString()}
              </p>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.sizes.map((size) => (
                    <button
                      key={size}
                      className="px-4 py-2 border border-gray-300 rounded-lg hover:border-[#AEAA9B] hover:bg-[#AEAA9B]/5 transition"
                    >
                      {size}
                    </button>
                  ))}
            ...
            {/* --- NEW: Replace static buttons with interactive Client Component --- */}
            <ProductVariantPanel
              productId={product._id}
              name={product.name}
              image={product.images[0]?.url || ""}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 pt-4 border-t border-gray-200">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <input
                  ...
                  className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#AEAA9B]"
                />
                <button className="px-4 py-2 bg-[#1A1A1A] text-white rounded-lg hover:bg-[#AEAA9B] transition">
                  Check
                </button>
              </div>
            </div>
    ...
    <Link href={`/product/${product.slug}`} className="group">
      <div className="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <div className="p-4">
          ...
          <p className="font-semibold text-[#AEAA9B] mt-1">
            ₹ {(product.salePrice != null && product.salePrice < product.price ? product.salePrice : product.price).toLocaleString()}
          </p>
        </div>
      </div>
