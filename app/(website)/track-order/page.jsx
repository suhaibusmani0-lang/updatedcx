...
function TrackOrderContent() {
  ...
  const doFetch = async (orderId, awb, isAutoRefresh = false) => {
    ...
    try {
      // --- SILENT BACKGROUND SYNC LOGIC ---
      // Agar user ne orderId dali hai lekin awb missing hai, toh ek baar Shipmozo se check kar lo
      if (orderId && !awb && !isAutoRefresh) {
        try {
          await fetch("/api/shipmozo/sync-awb", {
            method: "POST",
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-4xl px-4 py-10 md:py-14">
        ...
        <div className="mb-8 text-center" data-testid="track-order-header">
          <div className="mb-3 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#AEAA9B]/10">
            <Package className="h-7 w-7 text-[#AEAA9B]" />
          </div>
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm text-gray-900 outline-none focus:border-[#AEAA9B] focus:ring-2 focus:ring-[#AEAA9B]/20"
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#AEAA9B] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#a86641] disabled:cursor-not-allowed disabled:opacity-70 md:w-auto md:min-w-[180px]"
              ...
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-4 text-xs text-gray-500">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-none border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-700 hover:bg-gray-50"
                    disabled={refreshing}
                  ...
                  {tracking.orderId && (
                    <a
                      href={`/api/orders/${tracking.orderId}/download`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 rounded-none border border-[#AEAA9B] bg-[#AEAA9B] px-3 py-1 text-xs font-medium text-white hover:bg-[#9B9789]"
                      data-testid="track-order-print-receipt-button"
                    >
