    ...
    return (
      <section className="mx-auto flex min-h-[60vh] max-w-6xl items-center justify-center px-4 py-16 sm:px-6 lg:px-8">
        <div className="w-full max-w-md rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#F5E8DD] text-[#AEAA9B]">
            <Heart size={24} />
          </div>
          ...
          <button
            ...
            className="mt-6 inline-flex items-center justify-center rounded-full bg-[#AEAA9B] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#A56245]"
          >
            Sign in
          </button>
        </div>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-col gap-4 rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#AEAA9B]">Wishlist</p>
      ...
      {loading ? (
        ...
        <div className="rounded-[24px] border border-dashed border-gray-300 bg-white p-10 text-center shadow-sm">
          ...
          <Link
            href="/products"
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#AEAA9B] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#A56245]"
          >
            Browse products
            <ArrowRight size={16} />
          </Link>
            ...
            return (
              <article key={item._id || product?._id} className="overflow-hidden rounded-[24px] border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                <div className="relative aspect-square bg-[#FAF7F2]">
                  ...
                  {product?.badge ? (
                    <span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-[#AEAA9B] shadow-sm">
                      {product.badge}
                    </span>
                  ) : null}
                </div>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="rounded-full border border-gray-200 p-2 text-[#1A1A1A]/70 transition hover:border-[#AEAA9B] hover:bg-[#FFF5EE] hover:text-[#AEAA9B]"
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div>
                      <p className="text-lg font-semibold text-[#AEAA9B]">{formatPrice(product)}</p>
                    </div>
                    <Link
                      href={product?.slug ? `/product/${product.slug}` : "/products"}
                      className="inline-flex items-center gap-2 text-sm font-semibold text-[#1A1A1A] transition hover:text-[#AEAA9B]"
                    >
                      View
                      <ArrowRight size={14} />
                    </Link>
                  ...
                  <div className="mt-4 flex items-center gap-2 rounded-2xl bg-[#FAF7F2] px-3 py-2 text-sm text-[#1A1A1A]/70">
                    <ShoppingBag size={16} className="text-[#AEAA9B]" />
