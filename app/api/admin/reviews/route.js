...
async function revalidateProduct(productId) {
  try {
    const product = await ProductModel.findById(productId).select('slug').lean();
    if (product?.slug) {
    ...
    console.error('[Admin Reviews] Revalidation failed:', error);
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    await revalidateProduct(review.product);
