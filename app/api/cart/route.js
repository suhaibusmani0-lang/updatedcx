...
function normalizeVariant(v = {}) {
  return {
    ...
    price: typeof v.price === "number" && !Number.isNaN(v.price) ? v.price : null,
    salePrice: typeof v.salePrice === "number" && !Number.isNaN(v.salePrice) ? v.salePrice : null,
...
export async function POST(req) {
  try {
    ...
    const matchedVariant = (product.variants || []).find((candidate) => {
      const sameSku = variant.sku && candidate.sku === variant.sku;
      const sameSize = !variant.size || !candidate.size || candidate.size === variant.size;
      const sameColor = !variant.color || !candidate.color || candidate.color === variant.color;
      return sameSku || (sameSize && sameColor);
    });

    const variantStock = matchedVariant?.stock ?? product.stock;
    const variantPrice = matchedVariant?.price ?? product.salePrice ?? product.price;
    const variantSalePrice = matchedVariant?.salePrice ?? null;
    const variantImage = matchedVariant?.image || product.images[0]?.url || "";

    if (variantStock < quantity) return jsonRes(400, "Insufficient stock");
    ...
    if (existingItemIndex > -1) {
      ...
      if (variantStock < newQty) return jsonRes(400, "Insufficient stock");
      cart.items[existingItemIndex].qty = newQty;
      cart.items[existingItemIndex].price = variantSalePrice ?? variantPrice;
      cart.items[existingItemIndex].image = variantImage;
      ...
      cart.items.push({
        ...
        name: product.name,
        image: variantImage,
        price: variantSalePrice ?? variantPrice,
        qty: quantity,
        variant,
      });
    }
