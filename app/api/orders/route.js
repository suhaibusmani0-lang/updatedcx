...
export async function POST(req) {
  try {
    ...
    function normalizeOrderVariant(v = {}) {
      return { size: String(v.size || "").trim(), color: String(v.color || "").trim(), sku: String(v.sku || "").trim() };
    }

    function findMatchedVariant(product, variant) {
      if (!product.variants || !Array.isArray(product.variants)) return null;
      return product.variants.find((candidate) => {
        const sameBySku = variant.sku && candidate.sku === variant.sku;
        const sameBySize = !variant.size || !candidate.size || candidate.size === variant.size;
        const sameByColor = !variant.color || !candidate.color || candidate.color === variant.color;
        return sameBySku || (sameBySize && sameByColor);
      }) || null;
    }
    ...
    for (const item of items) {
      ...
      const variant = normalizeOrderVariant(item.variant || {});
      const matchedVariant = findMatchedVariant(product, variant);
      const availableStock = matchedVariant?.stock ?? product.stock;
      if (availableStock < item.qty) return jsonRes(400, `Insufficient stock for ${item.name}`);
    }
    ...
    for (const item of items) {
      ...
      if (matchedVariant) {
        await ProductModel.updateOne(
          { _id: productId, "variants._id": matchedVariant._id },
          { $inc: { stock: -item.qty, "variants.$.stock": -item.qty } }
        );
      } else {
