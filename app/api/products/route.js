...
export async function GET(req) {
  try {
    ...
    if (category) {
      const categoryDoc = await CategoryModel.findOne({ slug: category, isActive: true, isDeleted: false }).select("_id");
      if (categoryDoc) {
    ...
    const [products, total] = await Promise.all([
      ProductModel.find(query)
        .select("name slug price salePrice images category ratings isFeatured isNewArrival isBestSeller")
