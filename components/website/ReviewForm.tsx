  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 pt-6 border-t border-gray-100">
      ...
      <textarea
        ...
        rows={3}
        className="w-full border border-gray-200 rounded-xl p-3 text-sm outline-none focus:border-[#AEAA9B] mb-3"
      />
      {message && <p className={`text-sm mb-3 ${message.includes("success") ? "text-green-600" : "text-red-500"}`}>{message}</p>}
      <button
        type="submit"
        disabled={loading}
        className="px-6 py-2.5 bg-[#1A1A1A] text-white text-sm rounded-xl hover:bg-[#AEAA9B] transition-colors disabled:opacity-50"
      >
