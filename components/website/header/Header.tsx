...
export default function Header() {
  ...
  // 🔥 BUG FIX: Is code ko comment kar diya taaki scroll karte hi search bar band na ho
  /*
  useEffect(() => {
  ...
  */
  ...
  // Toggle mobile search - 🔥 BUG FIX: Top pe scroll add kar diya aur flow correct kiya
  const toggleMobileSearch = () => {
    ...
    if (willOpen) {
      ...
      setMobileSearchProducts([]);
      setMobileSearchCategories([]);
      // Click hone par smoothly top par bhejega
    ...
    <>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-5 py-4 sm:py-6">
              ...
              {auth ? (
                ...
                >
                  <Heart
                    ...
                    fill={wishlistCount > 0 ? "#C1121F" : "none"}
                  />
                  <span className="mt-1 hidden sm:inline">Wishlist</span>
                  <span
                    className={`absolute -top-1 -right-2 sm:-right-3 ${
                      wishlistCount > 0 ? "bg-[#C1121F] text-white" : "bg-gray-100 text-gray-700"
                    } text-[9px] rounded-full min-w-3.5 h-3.5 px-1 flex items-center justify-center font-bold shadow-sm`}
                  >
                ...
                <a
                  ...
                  className="hidden sm:flex flex-col items-center text-[10px] sm:text-xs hover:text-gray-600 relative text-gray-700"
                >
                  <Heart size={20} className="sm:w-[22px] sm:h-[22px] transition-colors text-gray-700" fill="none" />
                  <span className="mt-1 hidden sm:inline">Wishlist</span>
                </a>
              )}

              {/* Cart with live count -> UPDATED TO RED BADGE */}
              <button
                onClick={() => setCartOpen(true)}
                className="flex flex-col items-center text-[10px] sm:text-xs hover:text-gray-600 relative"
                aria-label="Cart"
              >
                <ShoppingBag size={20} className={`sm:w-[22px] sm:h-[22px] ${cartCount > 0 ? "text-[#1A1A1A]" : ""}`} />
                <span className="mt-1">Cart</span>
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-2 sm:-right-3 bg-[#C1121F] text-white text-[9px] rounded-full min-w-3.5 h-3.5 px-1 flex items-center justify-center font-bold shadow-sm">
          ...
          >
            <form onSubmit={handleMobileSearch} className="w-full relative">
              <div className="flex items-center border-2 border-gray-300 rounded-lg bg-white px-3 focus-within:border-[#AEAA9B] transition-colors">
                <input
                  ref={searchInputRef}
                  type="text"
                ...
                <button
                  type="submit"
                  className="p-2 hover:text-[#AEAA9B] transition-colors"
      ...
      >
        ...
        <nav className="flex flex-col">
          {menuData.map((item) => {
            ...
            return (
              ...
              >
                {/* Entire row is clickable now if it has subcategories */}
                <div className="flex items-center">
                  {hasCategories ? (
                    <button
                      onClick={() => {
                      ...
                      className="flex flex-1 items-center justify-between px-5 py-4 hover:bg-gray-50 text-left"
                        ...
                        return (
                          <div key={cat.title}>
                            <div className="flex items-center">
                              {hasSubLinks ? (
                                <button
                                  onClick={() => setExpandedSubMenu(subExpanded ? null : subKey)}
                                  className="flex flex-1 items-center justify-between px-8 py-3 hover:bg-white text-left"
                                >
                            ...
                            {hasSubLinks && subExpanded && (
                              <ul className="bg-white">
                                {/* "View All" link added so users can still reach the category page */}
                                <li>
                                  <Link
                                    href={cat.href}
                                    onClick={() => setMobileMenu(false)}
                                    className="block px-12 py-2 text-sm font-medium text-[#1A1A1A] hover:bg-gray-50"
                                  >
                                    View All {cat.title}
                                  </Link>
                                </li>
        ...
        <div className="mt-auto border-t p-5 flex flex-col gap-4">
          ...
          {/* Mobile Wishlist Link -> UPDATED TO RED BADGE */}
          {auth ? (
            <Link
              href="/wishlist"
              onClick={() => setMobileMenu(false)}
            ...
            >
              <span className="flex items-center gap-2">
                <Heart
                  size={18}
                  className={wishlistCount > 0 ? "text-[#C1121F]" : "text-gray-700"}
                  fill={wishlistCount > 0 ? "#C1121F" : "none"}
                />
                Wishlist
              </span>
            ...
            >
              <span className="flex items-center gap-2">
                <Heart size={18} className="text-gray-700" fill="none" />
                Wishlist
              </span>
            </a>
          )}
